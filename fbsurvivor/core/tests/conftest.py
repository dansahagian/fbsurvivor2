...
@pytest.fixture(autouse=True)
def seasons(db):
    return SeasonFactory.create_batch(size=2, year=factory.Iterator([2019, 2020]))
...
def weeks(db, seasons):
    ...
    return WeekFactory.create_batch(
        size=5, season=seasons[1], lock_datetime=factory.Iterator([lw, lw, lw, lw, nw])
    )


@pytest.fixture(autouse=True)
def teams(db, seasons):
    return TeamFactory.create_batch(size=5, season=seasons[1])
